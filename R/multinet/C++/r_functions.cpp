...
nodes(const RMLNetwork& rmnet, const CharacterVector& layer_names)
{
    ...
    for (auto l: *mnet->layers())
    {
        ...
        for (auto node: *l->vertices())
        {
            actor.push_back(node->name);
...
edges_idx(const RMLNetwork& rmnet)
{
    ...
    for (auto l: *mnet->layers())
    {
        ...
        {
            from.push_back(nodes->get_index(edge->v1)+num_nodes+1);
            to.push_back(nodes->get_index(edge->v2)+num_nodes+1);
...
DataFrame
isDirected(const RMLNetwork& rmnet, const CharacterVector& layer_names1, const CharacterVector& layer_names2)
{
    auto mnet = rmnet.get_mlnet();
    std::vector<uu::net::AttributedSimpleGraph*> layers1 = resolve_layers(mnet,layer_names1);
    std::vector<uu::net::AttributedSimpleGraph*> layers2;
...
auto actors = uu::net::neighbors(mnet,actor,layers,mode);
...
if (mode_name == "all")
    ...
    stop("unexpected value: mode " + mode_name);
auto actors = uu::net::xneighbors(mnet,actor,layers,mode);
for (auto neigh: *actors) {
...
if (directed.size()==1) {
    for (size_t i=0; i<layer_names.size(); i++) {
        mnet->add_layer(std::string(layer_names[i]),directed[0]?uu::net::EdgeDir::DIRECTED:UNuu::net::EdgeDir::DIRECTED);
    }
}
    ...
    for (size_t i=0; i<layer_names.size(); i++) {
        mnet->add_layer(std::string(layer_names[i]),directed[i]?uu::net::EdgeDir::DIRECTED:UNuu::net::EdgeDir::DIRECTED);
    }
}
}
