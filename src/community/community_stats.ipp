                               ...
                               )
{
    ...
    if (p.second == 0) return 1.0; // all communities are of the same size, as there is only one community
    else return double(p.second)/p.first;
}
                              ...
                              )
{
    ...
    for (auto pair: actors)
    {
        ...
        if (num_layers > pair.second.begin()->second) continue;
        count++;
    }
