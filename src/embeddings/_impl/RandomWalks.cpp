...
namespace net
{
    std::vector<std::string> random_walks_sl(uu::net::Network *single_net, std::unordered_map<const uu::net::Vertex *, std::unordered_map<const uu::net::Vertex *, AliasTable<uu::net::Vertex>>> &sampling_map_sl, std::default_random_engine &generator, const uu::net::Vertex *node_second, int len_rand_walk, int numb_rand_walks)
    {
        ...
        for (int i = 0; i < numb_rand_walks; ++i)
        {
            ...
            if (single_net->edges()->neighbors(node_second, uu::net::EdgeMode::OUT)->size() == 0)
            {
            ...
            {
                //auto node_first = single_net->edges()->neighbors(node_second, uu::net::EdgeMode::IN)->get_at_random();
                auto node_first = node_second;
                if (len_rand_walk == 1)
                {
                    output.push_back(node_second->name + ". ");
                }
                                                ...
                                                std::default_random_engine &generator, double r, int len_rand_walk, int numb_rand_walks)
    {
        ...
        for (int i = 0; i < numb_rand_walks; ++i)
        {
            ...
            if (layer->edges()->neighbors(node_second, uu::net::EdgeMode::OUT)->size() == 0) {        //Guarantees node is not isolated maybe
                output.push_back(node_second->name + ". ");
                continue;
            }

            //auto node_first = layer->edges()->neighbors(node_second, uu::net::EdgeMode::IN)->get_at_random();
            auto node_first = node_second;
            auto node_sentence = std::string();
            ...
            else
            {
                ...
                for (int j = 0; j < (len_rand_walk-1); ++j)
                {
                    if (distribution(generator) < r)
                    {
                        ...
                        if (layer->edges()->neighbors(node_second, uu::net::EdgeMode::OUT)->size() == 0) {  
                            node_sentence.append(" " + node_second->name);      //Guarantees node is not isolated maybe
                            break;
                        }
                        //node_first = layer->edges()->neighbors(node_second, uu::net::EdgeMode::IN)->get_at_random(); // what if we pick a node without in connections                        
                        node_first = node_second;
                    }
            ...
            else
            {
                ...
                for (int j = 0; j < (len_rand_walk-1); ++j)
                {
                    ...
                    if (interlayer_map.at(node_second).at(layer).size() != 0 && distribution(generator) < r)
                    {
                        ...
                        //node_first = layer->edges()->neighbors(node_second, uu::net::EdgeMode::IN)->get_at_random();
                        node_first = node_second;
                    }
        ...
        for (int i = 0; i < numb_rand_walks; ++i)
        {
            ...
            else
            {
                ...
                for (int j = 0; j < (len_rand_walk - 1); ++j)
                {
                    ...
                    auto new_layer = interlayer_map.at(node_second).at(layer).alias_sampling(&generator);
                    if(new_layer->name != layer->name)
                    {
                        node_first = node_second;
                        layer = new_layer;
                    }
