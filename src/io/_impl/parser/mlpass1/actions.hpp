...
struct type_act
{
    ...
    on_success(It const& first, It const& last, T& net_type, Ctx const& ctx)
    {
        ...
        else throw core::WrongFormatException("unsupported network type: " + net_type);
    }
};
...
struct layer_def_act
{
    ...
    on_success(It const& first, It const& last, T& layer_spec, Ctx const& ctx)
    {
        ...
        if (meta.is_multiplex)
        {
            ...
            if (l) throw core::WrongFormatException("layer " + layer_name + " already declared");
        ...
        else
        {
            ...
            if (layer_name1 == layer_name2)
            {
                ...
                if (l) throw core::WrongFormatException("layer " + layer_name1 + " already declared");
