...
struct UniquePtrLT {
    bool operator() (const std::unique_ptr<const int>& x, const int*& y) const {return x.get()<y;}
};
struct UniquePtrEQ {
    bool operator() (const std::unique_ptr<const int>& x, const int*& y) const {return x.get()==y;}
};
...
TEST(core_datastructures_containers_UniquePtrSortedRandomSet_test, ptr_to_element)
{
