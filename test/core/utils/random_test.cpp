...
TEST(core_utils_test, random)
{
    ...
    std::set<std::size_t> el(vec.begin(), vec.end());

    EXPECT_EQ((std::size_t)8, el.size()) << "Duplicate elements selected";

    for (std::size_t i = 0; i < 10; i++)
    {
        std::size_t r = uu::core::irand(4);
        EXPECT_TRUE(0 <= r && r < 4);
    }
    ...
    for (std::size_t i = 0; i < 10; i++)
    {
        std::size_t r = uu::core::test(probs);
        EXPECT_TRUE(r == 3 || r == 4);
    }
}
