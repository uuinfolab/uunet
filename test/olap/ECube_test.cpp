...
TEST_F(olap_ECube_test, cube_functionality)
{
    ...
    EXPECT_EQ(E->order(), (std::size_t) 0);
    EXPECT_EQ(E->dsize(), std::vector<std::size_t>({}));
    ...
    EXPECT_EQ(E->order(), (std::size_t) 2);
    EXPECT_EQ(E->dsize(), std::vector<std::size_t> ({3, 2}));
    ...
    EXPECT_EQ(E->size(), (std::size_t) 3);

    auto e4 = E->add(v2.get(), vc1.get(), v1.get(), vc2.get()); // (added to all cells)
    EXPECT_EQ(E->cell(index)->size(), (std::size_t) 4);
    EXPECT_EQ(E->erase(e4), true); // (erased from all cells)
    EXPECT_EQ(E->cell(index)->size(), (std::size_t) 3);
}
...
TEST_F(olap_ECube_test, discretization_higher_orders)
{
    ...
    EXPECT_EQ(E->size(), (std::size_t) 2);
    EXPECT_EQ(E->cell(m00)->size(), (std::size_t) 1);
    EXPECT_EQ(E->cell(m01)->size(), (std::size_t) 2);
    ...
    EXPECT_EQ(E->size(), (std::size_t) 2);
    EXPECT_EQ(E->cell(m000)->size(), (std::size_t) 1);
    EXPECT_EQ(E->cell(m001)->size(), (std::size_t) 1);
    EXPECT_EQ(E->cell(m010)->size(), (std::size_t) 1);
    EXPECT_EQ(E->cell(m011)->size(), (std::size_t) 2);
