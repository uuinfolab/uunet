...
Flattening adds the edges from a number of layers to a target network/layer. For example, assume that \code{net} contains two layers, \code{l1} and \code{l2}. We can add a new layer \code{lf1} and add to it all edges in either \code{l1} or \code{l2} as follows:
\begin{lstlisting}[style=c++]
auto l = {l1, l2};
