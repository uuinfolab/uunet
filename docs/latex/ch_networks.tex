...
\end{lstlisting}
A network also allows to add attributes to its vertices and edges: the \code{attr} functions return attribute stores (see Section \ref{ch:core:attr}):
\begin{lstlisting}[style=c++]
g->vertices()->attr();
g->edges()->attr();
\end{lstlisting}
...
g2->edges()->add(v1, v1);
\end{lstlisting}
and a directed network allowing loops and multi-edges. Notice that a \code{MultiNetwork} has a different interface: the function \code{edges()->get()} returns a container of edges, not a single edge.
\begin{lstlisting}[style=c++]
...
l2->vertices()->add(v1);
\end{lstlisting}
The command \code{l2->vertices()->add("v1")} would create and add a different vertex.
