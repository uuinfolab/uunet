...
typedef TID
COMBFN  (TIDLIST *d, TIDLIST *s1, TIDLIST *s2, SUPP *w);
typedef int
ECLATFN (ECLAT *eclat);
...
static TID
isect (TIDLIST *dst, TIDLIST *src1, TIDLIST *src2,SUPP *muls)
{
    ...
    while (1)                     /* trans. id list intersection loop */
    {
        if      (*s1 < *s2)
        {
            s2++;    /* if one transaction id is larger, */
        }

        else if (*s1 > *s2)
        {
            s1++;    /* simply skip this transaction id */
        }

        else if (*s1 <   0)
        {
            break;    /* check for the sentinel */
        }
...
static int
rec_base (ECLAT *eclat, TIDLIST **lists, ITEM k, size_t x)
{
    ...
    if (sig_aborted())
    {
        return -1;    /* if execution was aborted, */
    }
            ...
            &&  isr_xable(eclat->report, 2))       /* another item can be added */
    {
        ...
        if (!proj)
        {
            return -1;    /* allocate list and element arrays */
        }
    ...
    for (r = 0; k != z; k += eclat->dir)
    {
        ...
        if (r <= 0)
        {
            continue;    /* check if item needs processing */
        }
    ...
    if (proj)
    {
        free(proj);    /* delete the list and element arrays */
    }
...
eclat_base (ECLAT *eclat)
{
    ...
    if (!c)
    {
        return -1;    /* transactions per item */
    }
    ...
    if (!lists)
    {
        return -1;    /* create initial tid list array */
    }
    ...
    if (!p)
    {
        free(lists);    /* allocate tid list elements */
        return -1;
    }
    ...
    for (i = 0; i < k; i++)       /* traverse the items / tid lists */
    {
        ...
        p += c[i];
        *p++ = (TID)-1;  /* skip space for transaction ids */
    ...
    for (i = m = 0; i < k; i++)   /* traverse the items / tid lists */
    {
        ...
        if (l->supp <  eclat->supp)
        {
            continue;    /* collect perfect exts. */
        }

        if (l->supp >= pex)
        {
            isr_addpex(eclat->report, i);
            continue;
        }
...
static TID
filter (TIDLIST *dst, TIDLIST *src, SUPP *muls)
{
...
static int
closed (ECLAT *eclat, TIDLIST *list, ITEM n)
{
    ...
    if (eclat->mode & ECL_VERT)   /* if to use vertical representation */
    {
        while (--n >= 0)            /* traverse the eliminated items */
        {
            ...
            while (1)                 /* test for a perfect extension */
            {
                if      (*s < *d)
                {
                    d++;    /* skip missing destination id */
                }

                else if (*s > *d)
                {
                    break;    /* if source id is missing, abort */
                }
    ...
    else                          /* if to use horiz. representation */
    {
        ...
        if (item < 31)              /* if item bits can be used */
        {
            ...
            if (m)
            {
                return 0;    /* if perf. ext found, 'not closed' */
            }
        ...
        *r = TA_END;                /* store a sentinel at the end */
        ...
        for (s = list->tids+1; *s >= 0; s++)
        {
            ...
            while (1)                 /* item list intersection loop */
            {
                if      (*t <  0)
                {
                    break;    /* check for the list sentinel */
                }

                else if (*t < *p)
                {
                    p++;    /* if one item id is larger, */
                }

                else if (*t > *p)
                {
                    t++;    /* simply skip this item id, */
                }
...
static int
maximal (ECLAT *eclat, TIDLIST *list, ITEM n)
{
    ...
    if (eclat->mode & ECL_VERT)   /* if to use vertical representation */
    {
        while (--n >= 0)            /* traverse the eliminated items */
        {
            ...
            for (w = 0; 1; )          /* test for a perfect extension */
            {
                if      (*s < *d)
                {
                    d++;    /* if one transaction id is larger, */
                }

                else if (*s > *d)
                {
                    s++;    /* skip missing destination id */
                }
...
static int
rec_tcm (ECLAT *eclat, TIDLIST **lists,
...
static int
rec_tid (ECLAT *eclat, TIDLIST **lists, ITEM k, size_t x)
{
    /* --- eclat recursion with tid lists */
    int     r;                    /* error status */
    ...
    if (eclat->mode & ECL_TAIL)   /* if to use tail to prune w/ repo. */
    {
        ...
        if (r)
        {
            return r;    /* if tail need not be processed, */
        }
    }                             /* abort the recursion */
    ...
    for (r = 0; k != z; k += eclat->dir)
    {
        ...
        if (l->item < 0)            /* if this list is for packed items */
        {
            ...
            if (r >= 0)
            {
                continue;    /* to the 16-items machine and mine, */
            }

            if (proj)
            {
                free(proj);    /* then go to the next trans. id list */
            }
...
eclat_tid (ECLAT *eclat)
{
    ...
    if (!lists)
    {
        return -1;    /* create initial tid list array and */
    }
    ...
    if (x < (size_t)n)
    {
        x = (size_t)n;    /* ensure enough transaction ids */
    }
            ...
            &&  (l->supp >= eclat->supp)) /* and there are packed items */
    {
        ...
        l->item = TA_END;
        i = 1;    /* mark list for the packed items */
    ...
    if (r >= 0)                   /* if no error occurred */
    {
        ...
        if (!i || (max < w))        /* if to report the empty set */
        {
            ...
            {
                for (n = tbg_cnt(eclat->tabag); n > 0; n--)
                {
                    tids[n] = n;
                }
...
bit_init (void)
{
    ...
    if (bitcnt[1] != 0)
    {
        return;    /* check for an initialized table */
    }
...
static void
bit_isect (BITVEC *dst, BITVEC *src1, BITVEC *src2, TID n)
{
    ...
    {
        s = *s1++;
        m = *s2++;       /* traverse the bytes of each block */
        ...
        for ( ; m != 0; s >>= 8, m >>= 8)
        {
            ...
            if (b < 32)
            {
                continue;    /* add output bits for current byte */
            }

            b -= 32;
            *d++ = o;        /* if a bit block is full, store it */
    ...
    if (b > 0)
    {
        *d = o;    /* store the last bit vector block */
    }
}  /* bit_isect() */
...
bit_isect (BITVEC *dst, BITVEC *src1, BITVEC *src2, TID n)
{
    ...
    for (o = 0, b = 0; n > 0; n--)   /* traverse the bit vector blocks */
    {
        for (s = *s1++, m = *s2++; m != 0; )
        {
            ...
            s >>= 1;
            m >>= 1;         /* get the next source and mask bit */
...
static int
rec_bit (ECLAT *eclat, BITVEC **vecs, ITEM k, TID n)
{
            ...
            &&  isr_xable(eclat->report, 2))       /* another item can be added */
    {
        ...
        if (!proj)
        {
            return -1;    /* allocate bit vectors and array */
        }
    }                             /* (memory for conditional databases) */
    ...
    if (proj)
    {
        free(proj);    /* delete bit vectors and array */
    }
...
eclat_bit (ECLAT *eclat)
{
    ...
    if (!vecs)
    {
        return -1;    /* create initial bit vector array */
    }
...
static int
rec_tab (ECLAT *eclat, TIDLIST **lists, ITEM k, size_t x)
{
...
static int
rec_simp (ECLAT *eclat, TID *tids, SUPP n, ITEM k)
{
    ...
    {
        row = eclat->tab[k];
        s = 0; /* filter tids with item's table row */
        ...
        for (d = dst, p = tids; *p >= 0; p++)
            if ((w = row[*p]) > 0)    /* compute the item set support */
            {
                s += w;
                *d++ = *p;
            }    /* and the reduced trans. id list */

        if (s < eclat->supp)
        {
            continue;    /* skip infrequent items and */
        }

        if ((w = (SUPP)(d-dst)) >= pex)   /* collect perfect extensions */
        {
            isr_addpex(eclat->report, k);
            continue;
        }
...
eclat_simp (ECLAT *eclat)
{
    ...
    if (!eclat->tab)
    {
        return -1;    /* allocate working memory */
    }
...
static void
build_trg (TRGLIST **lists, TIDRANGE **next,
           TABAG *tabag, TID min, TID max, ITEM off)
{
    ...
    if (min > max)
    {
        return;    /* check for an empty trans. range */
    }
    ...
    if (off <= 0)                 /* if at first item in transactions */
    {
        ...
        for (k = min; min <= max; min++)
        {
            ...
            if (i >= 0)
            {
                break;    /* if it is not packed, abort loop */
            }
        ...
        if (min > k)                /* if the trans. range is not empty */
        {
            ...
            if (min > max)
            {
                return;    /* recursively build trans. ranges, */
            }
    ...
    do                            /* traverse the longer transactions */
    {
        ...
        for (k = min; ++min <= max; )    /* while not at end of section */
        {
            ...
            if (i != item)
            {
                break;    /* if the item differs, abort loop */
            }
        ...
        r = next[item]++;
        item = i; /* and create a new trans. id range */
...
static TID
isect_trg (TRGLIST *dst, TRGLIST *src1, TRGLIST *src2)
{
    ...
    while (1)                     /* range list intersection loop */
    {
        ...
        else if (s2->max < s1->min)      /* check for the sentinel */
        {
            if ((++s2)->min < 0)
            {
                break;    /* after advancing the range */
            }
        }
        ...
        else                        /* if the transaction ranges overlap */
        {
            ...
            {
                p = s1;
                ++d;            /* get/create a new trans. range */
...
static TID
filter_trg (TRGLIST *dst, TRGLIST *src1, TRGLIST *src2)
{
...
static int
rec_trg (ECLAT *eclat, TRGLIST **lists, ITEM k, size_t x)
{
    ...
    for (r = 0; k != z; k += eclat->dir)
    {
        ...
        if (l->item < 0)            /* if this list is for packed items */
        {
            ...
            if (proj)
            {
                free(proj);    /* then go to the next tid range list */
            }
...
eclat_trg (ECLAT *eclat)
{
    ...
    if (!lists)
    {
        return -1;    /* create initial range lists array */
    }
...
static int
odclo (ECLAT *eclat, TALIST *list, ITEM item)
{
    /* --- check for a closed item set */
    TID        k;                 /* loop variable */
    ...
    while ((UITEM)*p <= (UITEM)item)
    {
        p++;    /* find item added last */
    }

    while ((UITEM)*q <= (UITEM)item)
    {
        q++;    /* in both transactions */
    }
    ...
    for (d = eclat->cand; (*p >= 0) && (*q >= 0); )
    {
        ...
        else if ((UITEM)*p > (UITEM)*q)
        {
            q++;    /* skip smaller items */
        }
    ...
    *d = TA_END;                  /* terminate the candidate list */
    ...
    for (k = list->cnt-1; --k > 0; )
    {
        ...
        while (1)                   /* item list intersection loop */
        {
            ...
            {
                *d++ = *s++;    /* skip smaller item identifier and */
                p++;
            }
        }                           /* collect perfect ext. candidates */
...
static int
odmax (ECLAT *eclat, TALIST *list, ITEM item)
{
    /* --- check for a maximal item set */
    TID        k;                 /* loop variable */
...
static SUPP
rec_odcm (ECLAT *eclat, TALIST **lists, ITEM k)
{
...
#endif                        /* abort the recursion */
    l = lists[k];
    m = 0;          /* get the trans. list to process */
    ...
    if (m <= 0)
    {
        return 0;    /* if no frequent items found, abort */
    }
    ...
    for (max = w = 0, i = 0; i < k; i++)
    {
        ...
        if (p->supp <= 0)
        {
            continue;    /* but skip all eliminated items */
        }
        ...
        if (odclo(eclat, p, i))     /* if the current set may be closed */
        {
                    ...
                    :   ((w < eclat->supp) && odmax(eclat, p, i)))
            {
                ...
                if (r < 0)
                {
                    break;    /* if current set is closed/maximal, */
                }
        ...
        p->supp = 0;
        p->cnt = 0;    /* reinitialize the transaction list */
    }
...
static int
rec_odfx (ECLAT *eclat, TALIST **lists, ITEM k)
{
...
static int
rec_odro (ECLAT *eclat, TALIST **lists, ITEM k)
{
    ...
    for (inv = eclat->cand, i = m = 0; i < k; i++)
    {
        if (supp[i] <  eclat->supp) /* traverse items and their support */
        {
            supp[i] = -1;
            continue;
        } /* eliminate infrequent items an */

        if (supp[i] >= pex)         /* collect perfect extension items */
        {
            supp[i] = -1;
            isr_addpex(eclat->report,lists[i]->item);
            continue;
        }
    ...
    if (++i < k)
    {
        k = i;    /* and compute limit for item loop */
    }
            ...
            &&  (m >= ((eclat->fim16) ? 20 : 6)))    /* and enough items left */
    {
        ...
        if (!dst)
        {
            return -1;    /* allocate memory for projection */
        }
            ...
            && (eclat->dir > 0))          /* and forward processing direction */
    {
        ...
        if (r < 0)
        {
            return r;    /* and check for an error */
        }
    }
    ...
    for (r = 0; i < k; i++)       /* traverse the items/trans. lists, */
    {
        ...
        l->supp = 0;
        l->cnt = 0;    /* reinitialize the transaction list */
    }
...
eclat_ocd (ECLAT *eclat)
{
    ...
    if (!lists)
    {
        return -1;    /* create the trans. id list array */
    }
    ...
    l->item  = k;
    l->cnt = n;     /* for a dummy item (> all items) */
    ...
    free(tras);
    free(lists);      /* deallocate the transaction array */
    return r;                     /* return the error status */
}  /* eclat_ocd() */
...
static TID
cmpl (TIDLIST *dst, TIDLIST *src1, TIDLIST *src2, SUPP *muls)
{
    ...
    while (1)                     /* trans. id list difference loop */
    {
        ...
        {
            break;    /* collect elements of second source */
        }

        else
        {
            s1++;    /* that are not in the first source */
            s2++;
        }
...
static TID
diff (TIDLIST *dst, TIDLIST *src1, TIDLIST *src2, SUPP *muls)
{
    ...
    while (1)                     /* trans. id list difference loop */
    {
        ...
        {
            break;    /* remove all elements of the second */
        }

        else
        {
            s1++;    /* source from the first source */
            s2++;
        }
...
static int
rec_diff (ECLAT *eclat, TIDLIST **lists, ITEM k, TID x,
          COMBFN comb)
{
    ...
    for (r = 0; k != z; k += eclat->dir)
    {
        ...
        if (proj && (k > 0))        /* if another item can be added */
        {
            ...
            for (i = 0; i < k; i++)   /* traverse the preceding lists */
            {
                ...
                if (c > x)
                {
                    x = c;    /* collect the trans. id lists and */
                }
...
eclat_diff (ECLAT *eclat)
{
    ...
    for (i = m = 0; i < k; i++)   /* traverse the items / tid lists */
    {
        ...
        if (n > z)
        {
            z = n;    /* find maximum trans. id list length */
        }
...
static int
rec_tree (ECLAT *eclat, TALIST **lists, ITEM k)
{
    /* --- occ. deliver w/o reordering */
    ...
    for (i = m = 0; i < k; i++)   /* traverse the items */
    {
        ...
        {
            m += 1;    /* eliminate infrequent items */
        }
    ...
    for (i = 0; i < k; i++)       /* traverse the items and */
    {
        ...
        if (p->supp >= eclat->supp)
        {
            ist_setsupp(eclat->istree, i, p->supp);
        }
    ...
    for (i = 0; i < k; i++)       /* traverse the items */
    {
        ...
        if (p->supp <= 0)
        {
            continue;    /* skip all eliminated items */
        }
        ...
        p->supp = 0;
        p->cnt = 0;    /* reinit. the transaction list */
    }
              ...
              int algo, int mode)
{
    ...
    else if (target & ECL_GENERAS)
    {
        target = ISR_GENERAS;
    }

    else if (target & ECL_MAXIMAL)
    {
        target = ISR_MAXIMAL;
    }

    else if (target & ECL_CLOSED)
    {
        target = ISR_CLOSED;
    }
    ...
    if (!eclat)
    {
        return NULL;    /* create an eclat miner */
    }
...
eclat_delete (ECLAT *eclat, int deldar)
{
    ...
    if (deldar)                   /* if to delete data and reporter */
    {
        if (eclat->report)
        {
            isr_delete(eclat->report, 0);
        }

        if (eclat->tabag)
        {
            tbg_delete(eclat->tabag,  1);
        }
    }                             /* delete if existing */
...
int
eclat_data (ECLAT *eclat, TABAG *tabag, int mode, int sort)
{
    ...
    if (!(mode & ECL_NORECODE))   /* if to sort and recode the items */
    {
        ...
        if (m < 0)
        {
            return E_NOMEM;    /* recode items and transactions */
        }

        if (m < 1)
        {
            return E_NOITEMS;    /* and check the number of items */
        }
...
eclat_mine (ECLAT *eclat, ITEM prune, int order)
{
            ...
            &&  ((e <= RE_NONE) || (e >= RE_FNCNT)))
    {
        ...
        if (r < 0)
        {
            return E_NOMEM;    /* search for frequent item sets */
        }
    ...
    else                          /* if rules or rule-based evaluation */
    {
        ...
        if (r < 0)
        {
            return E_NOMEM;    /* then clean up the work memory */
        }
...
static ITEM
getbdr (char *s, char **end, double **border)
{
    ...
    *border = b = (double*)malloc((size_t)++k *sizeof(double));
    ...
    if (!b)
    {
        return -1;    /* allocate a support border */
    }
    ...
    for (i = 0; i < k; i++)       /* traverse the parameters */
    {
        ...
        if (*end == s)
        {
            break;    /* check for an empty parameter */
        }
...
static int
setbdr (ISREPORT *report, SUPP w, ITEM zmin,
        double **border, ITEM n)
{
    ...
    while (--n >= 0)              /* traverse the support values */
    {
        ...
        if (isr_setbdr(report, n+zmin, (RSUPP)s) < 0)
        {
            return -1;
        }
...
int main (int argc, char *argv[])
{
    ...
    for (i = 1; i < argc; i++)    /* traverse the arguments */
    {
        ...
        if ((*s == '-') && *++s)    /* -- if argument is an option */
        {
            while (*s)                /* traverse the options */
            {
                ...
                case 'm':
                    zmin   = (ITEM)strtol(s, &s, 0);
                    break;

                case 'n':
                    zmax   = (ITEM)strtol(s, &s, 0);
                    break;
    ...
    if (zmin   < 0)
    {
        error(E_SIZE, zmin);    /* check the size limits */
    }

    if (zmax   < 0)
    {
        error(E_SIZE, zmax);    /* and the minimum support */
    }

    if (smin   > 100)
    {
        error(E_SUPPORT, smin);
    }
    ...
    if (!eclat)
    {
        error(E_NOMEM);    /* create an eclat miner */
    }
    ...
    if (k)
    {
        error(k);    /* prepare data for eclat */
    }
    ...
    if (!report)
    {
        error(E_NOMEM);    /* and configure it */
    }
    ...
    if (k)
    {
        error(k);    /* prepare reporter for eclat */
    }
    ...
    if (k)
    {
        error(k, isr_tidname(report));    /* transaction ids */
    }
    ...
    if (k)
    {
        error(k);    /* find frequent item sets */
    }
    ...
    if (fn_psp)                   /* if to write a pattern spectrum */
    {
        ...
        if (!twrite)
        {
            error(E_NOMEM);    /* open the output file */
        }
