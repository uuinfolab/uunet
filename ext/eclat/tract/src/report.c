...
static int
getsd (const char *s, const char **end)
{
    ...
    if (k > 32)
    {
        k = 32;    /* limit   the number of digits */
    }

    *end = s;
    return k;           /* return  the number of digits */
}  /* getsd() */

/*--------------------------------------------------------------------*/

void
isr_flush (ISREPORT *rep) // MM: removed static
{
...
static int
isr_puts (ISREPORT *rep, const char *s)
{
...
static void
isr_putsn (ISREPORT *rep, const char *s, int n)
{
    /* --- write a character string */
    ...
    while (n > 0)                 /* while there are characters left */
    {
        ...
        if (k >= n)                 /* if the string fits into buffer */
        {
            ...
            rep->next += n;
            break;    /* simply copy the string into */
...
int
isr_intout (ISREPORT *rep, ptrdiff_t num)
{
    ...
    if (num <= PTRDIFF_MIN)       /* treat minimum as a special case */
    {
        ...
        isr_putsn(rep, "-9223372036854775808", 20);
        return 20;
#else                       /* if 32 bit system */
        isr_putsn(rep, "-2147483648", 11);
        return 11;
...
int
mantout (ISREPORT *rep, double num, int digits, int ints)
{
    ...
    x = floor(num);
    y = num-x;    /* split into integer and fraction */
                      ...
                      &&  ((e > d) ? *(e-1) & 1 : floor(x/2) >= x/2)))
    {
        ...
        if ((s <= d) && ((x += 1) >= pows[i]))
            if (--e <= d+1)
            {
                e = d;    /* if all decimals have been adapted, */
            }
    ...
    if (e > d)                    /* if there are decimal places, */
    {
        ...
        if (e > d)
        {
            e++;    /* if there are no decimals left, */
        }
...
int
isr_numout (ISREPORT *rep, double num, int digits)
{
    ...
    if (digits > 32)
    {
        digits = 32;    /* limit the number of sign. digits */
    }
    ...
    {
        num /= pows[1-k];
        e += -1-k;/* on failure adapt the mantissa */
    ...
    if (e == 0)
    {
        return n;    /* if no exponent, abort the function */
    }

    isr_putc(rep, 'e');
    n += 2;   /* print an exponent indicator */
    isr_putc(rep, (e < 0) ? '-' : '+');
...
int
isr_wgtout (ISREPORT *rep, RSUPP supp, double wgt)
{
    ...
    for (s = rep->iwf; *s; )      /* traverse the output format */
    {
        ...
        t = s++;
        k = getsd(s,&s);   /* get the number of signif. digits */
        ...
        case 'm':
            n += isr_numout(rep, wgt/(double)supp, k);
            break;

        case  0 :
            --s;            /* print the requested quantity */

        default :
            isr_putsn(rep, t, k = (int)(s-t));
            n += k;
            t = s;
            break;
...
void
isr_tidflush (ISREPORT *rep) // MM: removed static
{
    /* --- flush the output buffer */
    assert(rep);                  /* check the function arguments */
...
static void
isr_tidputc (ISREPORT *rep, int c)
{
    /* --- write a single character */
    assert(rep);                  /* check the function arguments */
...
static void
isr_tidputs (ISREPORT *rep, const char *s)
{
    /* --- write a character string */
    assert(rep);                  /* check the function arguments */
...
static void
isr_tidputsn (ISREPORT *rep, const char *s, int n)
{
    /* --- write a character string */
    int k;                        /* number of chars in buffer */
...
static void
isr_tidout (ISREPORT *rep, TID tid)
{
    ...
    if (rep->ints                 /* if pre-formatted ints. available */
            ...
            && (tid <= rep->imax))
    {
        isr_tidputs(rep, rep->ints[tid -rep->imin]);
        return;
    }
...
static void
isr_occout (ISREPORT *rep, ITEM occ)
{
    ...
    if (rep->ints                 /* if pre-formatted ints. available */
            ...
            && ((TID)occ <= rep->imax))
    {
        isr_tidputs(rep, rep->ints[occ -rep->imin]);
        return;
    }
...
is_cmp (const void *a, const void *b, void *d)
{
    ...
    x = (ITEM*)a;
    y = (ITEM*)b;   /* get/type the item set pointers */
    n = *x++;                     /* if the item set sizes differ, */

    if (n != *y++)
    {
        return 1;    /* the item sets are not equal */
    }

    while (--n >= 0)              /* traverse and compare the items */
        if (x[n] != y[n])
        {
            return 1;    /* if an item differs, abort */
        }
...
static int
is_isgen (ISREPORT *rep, ITEM item, RSUPP supp)
{
    ...
    if (rep->cnt > 0)             /* if the current set is not empty */
    {
        ...
        for (++i; --i >= 0; )       /* traverse the items in the set */
        {
            b = p[i];
            p[i] = a;
            a = b;/* get next subset (next hold-out) */

            if (a == item)
            {
                continue;    /* do not exclude the new item */
            }
        ...
        if (i >= 0)
        {
            return 0;    /* if subset was found, no generator */
        }
    ...
    if (!s)
    {
        return -1;    /* add the new set to the repository */
    }
...
ISREPORT*
isr_createx (ITEMBASE *base, ITEM max)
{
...
int
isr_delete (ISREPORT *rep, int delis)
{
    ...
    if (rep->out)
    {
        free(rep->out);    /* delete the item set output buffer */
    }
    ...
    if (rep->clomax)
    {
        cm_delete(rep->clomax);
    }

    if (rep->gentab)
    {
        st_delete(rep->gentab);
    }
    ...
    if (rep->buf)
    {
        free(rep->buf);    /* delete file write buffers */
    }
...
int
isr_settarg (ISREPORT *rep, int target, int mode, int dir)
{
    ...
    else if (target & ISR_GENERAS)
    {
        target = ISR_GENERAS;
    }

    else if (target & ISR_MAXIMAL)
    {
        target = ISR_MAXIMAL;
    }

    else if (target & ISR_CLOSED)
    {
        target = ISR_CLOSED;
    }
...
void
isr_setsupp (ISREPORT *rep, RSUPP smin, RSUPP smax)
{
...
void
isr_setsize (ISREPORT *rep, ITEM zmin, ITEM zmax)
{
...
RSUPP
isr_setbdr (ISREPORT *rep, ITEM size, RSUPP supp)
{
    ...
    if (size >= n)                /* if to enlarge the border array */
    {
        ...
        if (n <= size)
        {
            n = size+1;    /* compute the new array size */
        }
        ...
        if (!p)
        {
            return -1;    /* enlarge the border array */
        }
...
int
isr_setfmtx (ISREPORT *rep, int scan, CCHAR *hdr, CCHAR *sep,
             CCHAR *imp, CCHAR *info, CCHAR *iwf)
{
    ...
    if (!d)
    {
        return -1;    /* create a buffer for the strings */
    }

    for (rep->str  = d, s = hdr;  *s; )
    {
        *d++ = *s++;
    }
...
void
isr_seteval (ISREPORT *rep, ISEVALFN evalfn, void *data,
...
void
isr_setrepo (ISREPORT *rep, ISREPOFN repofn, void *data)
{
...
void
isr_setrule (ISREPORT *rep, ISRULEFN rulefn, void *data)
{
...
int
isr_prefmt (ISREPORT *rep, TID min, TID max)
{
    ...
    if (max < 0)
    {
        return 0;    /* if no preformatting, abort */
    }

    if (min < 0)
    {
        min = 0;    /* ensure a non-negative minimum */
    }

    rep->imin = min;              /* note the range of integers */
    rep->imax = max;              /* that will be pre-formatted */
    max += 1;
    z = max+max;        /* one digit plus end-of string */
    ...
    *b = 0;   /* and get pointer to last digit */
    ...
    for (t = min; t < max; t++)   /* traverse the transaction ids */
    {
        ...
        if (c < b)
        {
            b = c;    /* adapt the start of the number */
        }
...
int
isr_open (ISREPORT *rep, FILE *file, const char *name)
{
    /* --- open an output file */
    assert(rep);                  /* check the function arguments */
    ...
    else                          /* if a proper name is given */
    {
        ...
        if (!file)
        {
            return E_FOPEN;    /* open file with given name */
        }
...
isr_close (ISREPORT *rep)
{
    ...
    if (!rep->file)
    {
        return 0;    /* check for an output file */
    }
...
int
isr_tidopen (ISREPORT *rep, FILE *file, const char *name)
{
...
void
isr_tidcfg (ISREPORT *rep, TID tracnt, ITEM miscnt)
{
...
isr_setup (ISREPORT *rep)
{
    ...
    if (rep->out)
    {
        free(rep->out);    /* delete an existing output buffer */
    }
    ...
    if (!rep->out)
    {
        return E_NOMEM;    /* create an output buffer and */
    }
...
int
isr_add (ISREPORT *rep, ITEM item, RSUPP supp)
{
    ...
    else if (rep->gentab)         /* if a generator filter exists */
    {
        ...
        if (r <= 0)
        {
            return r;    /* add item set to the gen. filter */
        }
...
int
isr_addnc (ISREPORT *rep, ITEM item, RSUPP supp)
{
    /* --- add an item (only support) */
    assert(rep && (item >= 0)     /* check the function arguments */
           &&    (item < ib_cnt(rep->base)));
...
int
isr_addwgt (ISREPORT *rep, ITEM item, RSUPP supp, double wgt)
{
...
int
isr_addpex (ISREPORT *rep, ITEM item)
{
...
void
isr_addpexpk (ISREPORT *rep, ITEM bits)
{
    /* --- add a perfect extension */
...
void
isr_remove (ISREPORT *rep, ITEM n)
{
...
isr_logrto (ISREPORT *rep, void *data)
{
    ...
    if (rep->cnt <= 1)
    {
        return 0;    /* if only one item, abort */
    }
...
double
isr_lrsize (ISREPORT *rep, void *data)
{
...
output (ISREPORT *rep)
{
    ...
    if (!rep->file)
    {
        return;    /* check for an output file */
    }
...
isr_report (ISREPORT *rep)
{
    ...
    if (rep->clomax)              /* if a closed/maximal filter exists */
    {
        ...
        if (r >= s)
        {
            return 0;    /* check if item set is not closed */
        }

        if (r >= rep->sto)
        {
            return 0;    /* check whether to store item set */
        }
    ...
    if (rep->fast < 0)            /* if just to count the item sets */
    {
        ...
        m = 0;
        z = rep->cnt;        /* and init. the item set counter */
    ...
    if (rep->mode & ISR_NOEXPAND) /* if not to expand perfect exts. */
    {
        ...
        if (k > rep->zmax)
        {
            return 0;    /* the item set too large, abort */
        }
        ...
        if (rep->fast)
        {
            fastout(rep, 0);    /* report the expanded set */
        }

        else
        {
            output (rep);    /* (fast or normal output) */
        }
...
int
isr_reportv (ISREPORT *rep, double eval)
{
    /* --- report the current item set */
    assert(rep);                  /* check the function arguments */
...
int
isr_reportx (ISREPORT *rep, TID *tids, TID n)
{
    /* --- report the current item set */
...
int
isr_reporto (ISREPORT *rep, ITEM *occs, TID n)
{
    /* --- report the current item set */
    int r;                        /* result of function call */
...
int
isr_reprule (ISREPORT *rep, ITEM item,
...
int
isr_iset (ISREPORT *rep, const ITEM *items, ITEM n,
          RSUPP supp, double wgt, double eval)
{
    ...
    i = rep->cnt;
    rep->cnt = n;   /* note the number of items */
    isr_puts(rep, rep->hdr);      /* print the record header */
...
int
isr_isetx (ISREPORT *rep, const ITEM *items, ITEM n,
           const double *iwgts,
           RSUPP supp, double wgt, double eval)
{
    /* --- report an item set */
...
int
isr_rule (ISREPORT *rep, const ITEM *items, ITEM n,
...
int
isr_seqrule (ISREPORT *rep, const ITEM *items, ITEM n,
             RSUPP supp, RSUPP body, RSUPP head, double eval)
{
...
int
isr_sxrule (ISREPORT *rep, const ITEM *ante, ITEM n, ITEM cons,
            RSUPP supp, RSUPP body, RSUPP head, double eval)
{
    /* --- report a sequence rule */
    ITEM i;                       /* buffer for the item counter */
    ...
    i = rep->cnt;
    rep->cnt = n+1; /* note the number of items */
    isr_puts(rep, rep->hdr);      /* print the record header */
...
int
isr_extrule (ISREPORT *rep, const ITEM *items, ITEM n,
...
void
isr_prstats (ISREPORT *rep, FILE *out, ITEM min)
{
...
int
isr_addpsp (ISREPORT *rep, PATSPEC *psp)
{
    ...
    if (rep->psp)
    {
        return 1;    /* if pattern spectrum exists, abort */
    }
    ...
    if (!psp)                     /* if to create a pattern spectrum */
    {
        ...
        if (!psp)
        {
            return -1;    /* create a pattern spectrum */
        }
...
PATSPEC*
isr_rempsp (ISREPORT *rep, int delpsp)
{
    /* --- add a pattern spectrum */
    ...
    if (!delpsp)
    {
        return psp;    /* if not to delete it, return it */
    }

    if (psp)
    {
        psp_delete(psp);    /* delete existing pattern spectrum */
    }
...
int
isr_sinfo (ISREPORT *rep, RSUPP supp, double wgt, double eval)
{
    ...
    if (smax <= 0)
    {
        smax = 1;    /* avoid divisions by zero */
    }
    ...
    if (wmax <= 0)
    {
        wmax = 1;    /* avoid divisions by zero */
    }
...
//#define double 2
        ...
        case 'a':
            n += isr_numout(rep,      sdbl,       k);
            break;

        case 'q':
            n += isr_numout(rep,      smax,       k);
            break;
        ...
        case 'a':
            n += isr_intout(rep, (ptrdiff_t)supp);
            break;

        case 'q':
            n += isr_intout(rep, (ptrdiff_t)smax);
            break;
...
#undef double
        ...
        case 's':
            n += isr_numout(rep,      sdbl/smax,  k);
            break;

        case 'S':
            n += isr_numout(rep, 100*(sdbl/smax), k);
            break;
        ...
        case 'W':
            n += isr_numout(rep, 100* wgt,        k);
            break;

        case 'r':
            n += isr_numout(rep,      wgt /wmax,  k);
            break;

        case 'R':
            n += isr_numout(rep, 100*(wgt /wmax), k);
            break;

        case 'z':
            n += isr_numout(rep,      wgt *smax,  k);
            break;

        case 'e':
            n += isr_numout(rep,      eval,       k);
            break;

        case 'E':
            n += isr_numout(rep, 100* eval,       k);
            break;
...
int
isr_rinfo (ISREPORT *rep, RSUPP supp, RSUPP body, RSUPP head,
           double eval)
{
...
//#define double 2
        ...
        case 'a':
            n += isr_numout(rep,            supp,         k);
            break;

        case 'b':
            n += isr_numout(rep,            body,         k);
            break;

        case 'h':
            n += isr_numout(rep,            head,         k);
            break;
        ...
        case 'b':
            n += isr_intout(rep, (ptrdiff_t)body);
            break;

        case 'h':
            n += isr_intout(rep, (ptrdiff_t)head);
            break;
...
#undef double
        ...
        case 's':
            n += isr_numout(rep,      (double)supp/smax,  k);
            break;

        case 'S':
            n += isr_numout(rep, 100*((double)supp/smax), k);
            break;

        case 'x':
            n += isr_numout(rep,      (double)body/smax,  k);
            break;

        case 'X':
            n += isr_numout(rep, 100*((double)body/smax), k);
            break;

        case 'y':
            n += isr_numout(rep,      (double)head/smax,  k);
            break;

        case 'Y':
            n += isr_numout(rep, 100*((double)head/smax), k);
            break;

        case 'c':
            conf = (body > 0) ? (double)supp/(double)body : 0;
            n += isr_numout(rep,      conf,               k);
            break;

        case 'C':
            conf = (body > 0) ? (double)supp/(double)body : 0;
            n += isr_numout(rep, 100* conf,               k);
            break;

        case 'l':
            lift = ((body <= 0) || (head <= 0)) ? 0
                   : ((double)supp*smax) /((double)body*(double)head);
            n += isr_numout(rep,      lift,               k);
            break;

        case 'L':
            lift = ((body <= 0) || (head <= 0)) ? 0
                   : ((double)supp*smax) /((double)body*(double)head);
            n += isr_numout(rep, 100* lift,               k);
            break;
...
int
isr_xinfo (ISREPORT *rep, RSUPP supp, RSUPP body, RSUPP head,
           RSUPP salt, RSUPP halt, RSUPP join)
{
...
//#define double 2
        ...
        case 'H':
            n += isr_numout(rep,              halt,       k);
            break;

        case 'B':
            n += isr_numout(rep,              join,       k);
            break;

        case 'o':
            n += isr_numout(rep,              supp-join,  k);
            break;

        case 'O':
            n += isr_numout(rep,              salt-join,  k);
            break;
        ...
        case 'A':
            n += isr_intout(rep,  (ptrdiff_t) salt);
            break;
        ...
        case 'H':
            n += isr_intout(rep,  (ptrdiff_t) halt);
            break;

        case 'B':
            n += isr_intout(rep,  (ptrdiff_t) join);
            break;

        case 'o':
            n += isr_intout(rep,  (ptrdiff_t)(supp-join));
            break;

        case 'O':
            n += isr_intout(rep,  (ptrdiff_t)(salt-join));
            break;
...
#undef double
        ...
        case 't':
            n += isr_numout(rep,      (double)salt/smax,  k);
            break;

        case 'T':
            n += isr_numout(rep, 100*((double)salt/smax), k);
            break;
        ...
        case 'z':
            n += isr_numout(rep,      (double)halt/smax,  k);
            break;

        case 'Z':
            n += isr_numout(rep, 100*((double)halt/smax), k);
            break;

        case 'w':
            n += isr_numout(rep,      (double)join/smax,  k);
            break;

        case 'W':
            n += isr_numout(rep, 100*((double)join/smax), k);
            break;

        case 'u':
            x  = (double)(supp-join);
            n += isr_numout(rep,      (double)x   /smax,  k);
            break;

        case 'U':
            x  = (double)(supp-join);
            n += isr_numout(rep, 100*((double)x   /smax), k);
            break;
        ...
        case 'f':
            conf = (body > 0) ? (double)salt/(double)body : 0;
            n += isr_numout(rep,      conf,               k);
            break;
...
void
isr_getinfo (ISREPORT *rep, const char *sel, double *vals)
{
