...
typedef ITEM
SUBFN  (const TRACT  *t1, const TRACT  *t2, ITEM off);
typedef ITEM
SUBWFN (const WTRACT *t1, const WTRACT *t2, ITEM off);
...
appcmp (const void *p1, const void *p2, void *data)
{
    ...
    for ( ; 1; s1++, s2++)        /* traverse the strings */
    {
        ...
        if (d   != 0)
        {
            return d;    /* if strings differ, return result */
        }
...
appcode (CCHAR *s)
{
    ...
    if (i >= n)
    {
        return -1;    /* try to find appearance indicator */
    }
    ...
    if (*s)
    {
        return -1;    /* if no match, abort the function */
    }
...
ib_create (int mode, ITEM size, ...)
{
    ...
    if (size <= 0)
    {
        size = BLKSIZE;    /* check and adapt number of items */
    }
    ...
    if (!base)
    {
        return NULL;    /* create item base and components */
    }
...
ib_add (ITEMBASE *base, const void *name)
{
    ...
    if (itd == NULL)
    {
        return -1;    /* add the new item */
    }

    if (itd == EXISTS)
    {
        return -2;    /* to the identifier map */
    }
...
ib_add2ta (ITEMBASE *base, const void *name)
{
    ...
    if (!itd)                     /* get the item by its key/name */
    {
        ...
        if (itd == NULL)
        {
            return -1;    /* add a new item to the base */
        }
    ...
    if (t->size >= n)             /* if the transaction buffer is full */
    {
        ...
        if (!t)
        {
            return -1;    /* enlarge the transaction buffer */
        }

        t->items[base->size = n] = TA_END;
        base->tract = t;
...
ib_readsel (ITEMBASE *base, TABREAD *trd)
{
    ...
    while (1)                     /* read a simple list of items */
    {
        ...
        if (itd == NULL)
        {
            return base->err = E_NOMEM;
        }

        if (itd == EXISTS)
        {
            continue;    /* add new item, skip known */
        }
...
ib_read (ITEMBASE *base, TABREAD *trd, int mode)
{
    ...
    do                            /* read the transaction items */
    {
        ...
        if (!*b)                    /* if the field is empty, */
        {
            if (d == TRD_REC)
            {
                break;    /* the transaction must be empty */
            }
        ...
        if (!itd)                   /* if the item is not known yet */
        {
            if (base->app == APP_NONE)   /* if new items are to be ignored */
            {
                ...
                if (d == TRD_REC)
                {
                    break;    /* if at the end of a record, */
                }
...
int
ib_write (ITEMBASE *base, TABWRITE *twr, const char *wgtfmt, ...)
{
    /* --- write a transaction */
    va_list    args;              /* list of variable arguments */
           ...
           ITEM cnt, int dir, ITEM *map)
{
    ...
    if (max < 0)
    {
        max = SUPP_MAX;    /* adapt the maximum frequency */
    }

    if (cnt < 0)
    {
        cnt = ITEM_MAX;    /* adapt the maximum number of items */
    }
    ...
    if      (dir >  1)
    {
        cmp = asccmpx;    /* get the appropriate */
    }

    else if (dir >  0)
    {
        cmp = asccmp;    /* comparison function */
    }

    else if (dir >= 0)
    {
        cmp = nocmp;    /* (ascending/descending) */
    }

    else if (dir > -2)
    {
        cmp = descmp;    /* and sort the items */
    }

    else
    {
        cmp = descmpx;    /* w.r.t. their frequency */
    }
    ...
    if (n > cnt)
    {
        n = cnt;    /* limit the number of items */
    }
    ...
    if (!map)
    {
        return n;    /* if no map is provided, abort */
    }
    ...
    else                          /* if the items do not carry weights */
    {
        ...
        for (s = d = t->items; *s > TA_END; s++)
        {
            ...
            if (i >= 0)
            {
                *d++ = i;    /* remove all items to ignore */
            }
        }                           /* from the buffered transaction */
...
void
ib_trunc (ITEMBASE *base, ITEM cnt)
{
...
ta_create (const ITEM *items, ITEM n, SUPP wgt)
{
    ...
    if (!t)
    {
        return NULL;    /* allocate a new transaction */
    }

    t->wgt  = wgt;                /* set weight, size and marker */
    t->size = n;
    t->mark = 0;     /* and copy the items */
...
ta_sort (TRACT *t, int dir)
{
    ...
    if ((n = t->size) < 2)
    {
        return;    /* do not sort less than two items */
    }
...
ta_reverse (TRACT *t)
{
    ...
    if ((n = t->size) < 2)
    {
        return;    /* do not reverse less than two items */
    }
...
ta_pack (TRACT *t, int n)
{
    ...
    if (n <= 0)
    {
        return 0;    /* if no items to pack, abort */
    }

    if (n > 31)
    {
        n = 31;    /* pack at most 31 items */
    }

    for (s = t->items; *s > TA_END; s++)
        if (*s < n)
        {
            break;    /* find item with code < n */
        }
    ...
    for (b = 0; *s > TA_END; s++)
    {
        if      (*s < 0)
        {
            b |= *s;    /* traverse the remaining items */
        }
        ...
        {
            *++d = *s;    /* set bits for items with codes < n */
        }
    ...
    *p = b | TA_END;              /* store the packed items (bit rep.) */
    ...
    while (++d < s)
    {
        *d = TA_END;    /* clear rest of the transaction */
    }
...
ta_unpack (TRACT *t, int dir)
{
    ...
    if (*d <= TA_END)
    {
        return 0;    /* if there are none, abort */
    }
    ...
    for (i = k = 0, q = p = *d & ~TA_END; q; q >>= 1)
    {
        i += q & 1;
        k++;
    }          /* get and count the packed items */
...
ta_equal (const TRACT *t1, const TRACT *t2)
{
    ...
    for (a = t1->items, b = t2->items; *a > TA_END; a++, b++)
        if (*a != *b)
        {
            return -1;    /* abort if corresp. items differ */
        }
...
ta_cmp (const void *p1, const void *p2, void *data)
{
    ...
    for ( ; 1; a++, b++)          /* traverse the item arrays */
    {
        if (*a < *b)
        {
            return -1;    /* compare corresponding items */
        }

        if (*a > *b)
        {
            return +1;    /* and if one is greater, abort */
        }

        if (*a <= TA_END)
        {
            return 0;    /* otherwise check for the sentinel */
        }
...
int
ta_cmpoff (const void *p1, const void *p2, void *data)
{
    /* --- compare transactions */
    ITEM i, k;                    /* item buffers */
    ...
    if (i < k)
    {
        return -1;    /* compare items at given offset */
    }
...
int
ta_cmplim (const void *p1, const void *p2, void *data)
{
...
int
ta_cmpsfx (const void *p1, const void *p2, void *data)
{
...
int
ta_cmpsep (const void *p1, const void *p2, void *data)
{
    /* --- compare transaction suffixes */
    ITEM       i, k;              /* item buffers */
    const ITEM *a, *b;            /* to traverse the items */
...
int
ta_cmpx (const TRACT *t, const ITEM *items, ITEM n)
{
    /* --- compare transactions */
    ...
    for (i = t->items; items < end; i++, items++)
    {
        ...
        if (*i > *items)
        {
            return +1;    /* and abort the comparison */
        }
...
int
ta_cmpsz (const void *p1, const void *p2, void *data)
{
    /* --- compare transactions */
...
ITEM
ta_subset (const TRACT *t1, const TRACT *t2, ITEM off)
{
    ...
    if (*s <= TA_END)
    {
        return 0;    /* then traverse the destination */
    }
    ...
    for (d = t2->items +off; *d > TA_END; d++)
    {
        if (*d != *s)
        {
            continue;    /* try to find source start in dest. */
        }
        ...
        for (x = s+1, y = d+1; 1; y++)    /* compare the remaining items */
        {
            if (*x <= TA_END)
            {
                return (ITEM)(d -t2->items);
            }

            if (*y <= TA_END)
            {
                break;    /* all in source matched, subset, */
            }

            if (*x == *y)
            {
                x++;    /* end of destination, no subset */
            }
...
ITEM
ta_subwog (const TRACT *t1, const TRACT *t2, ITEM off)
{
    ...
    for (d = t2->items +off; *d > TA_END; d++)
    {
        ...
        x = s;
        y = d;               /* compare the remaining items */
...
wi_cmp (const WITEM *a, const WITEM *b)
{
    ...
    for (i = 0; 1; i++)           /* compare the items */
    {
        ...
        if (a[i].item < 0)
        {
            break;    /* check for the sentinel and */
        }
    ...
    for (i = 0; 1; i++)           /* compare the item weights */
    {
        ...
        if (a[i].item < 0)
        {
            return 0;    /* check for the sentinel and */
        }
    }                             /* abort if it is reached */
}  /* wi_cmp() */
...
wi_rec (WITEM *wia, ITEM n)
{
    ...
    do                            /* sections sort loop */
    {
        ...
        if      (x < l->item)
        {
            x = l->item;    /* try to find a */
        }

        else if (x > r->item)
        {
            x = r->item;    /* better pivot element */
        }
...
wta_create (ITEM size, SUPP wgt)
{
    ...
    t->size = 0;
    t->mark = 0;     /* and init. the transaction size */
...
wta_clone (const WTRACT *t)
{
    ...
    if (!c)
    {
        return NULL;    /* allocate a new transaction and */
    }
...
wta_add (WTRACT *t, ITEM item, float wgt)
{
    ...
    d->item = item;
    d->wgt = wgt; /* store item ientifier and weight */
}  /* wta_add() */
...
int
wta_cmpsz (const void *p1, const void *p2, void *data)
{
    /* --- compare transactions */
    ITEM a, b;                    /* transaction sizes */
...
ITEM
wta_subset (const WTRACT *t1, const WTRACT *t2, ITEM off)
{
    /* --- test for subset/subsequence */
...
ITEM
wta_subwog (const WTRACT *t1, const WTRACT *t2, ITEM off)
{
    /* --- test for subsequence w/o gaps */
...
wta_show (WTRACT *t, ITEMBASE *base)
{
    ...
    for (k = 0; k < t->size; k++) /* traverse the items */
    {
        ...
        if (base)
        {
            printf("%s/", ib_xname(base, t->items[k].item));
        }
...
tbg_create (ITEMBASE *base)
{
    ...
    if (!bag)
    {
        return NULL;    /* create a transaction bag/multiset */
    }
...
tbg_delete (TABAG *bag, int delib)
{
    ...
    if (bag->buf)
    {
        free(bag->buf);    /* delete buffer for surrogates */
    }
...
tbg_clone (TABAG *bag)
{
    ...
    if (dst)
    {
        tbg_copy(dst, bag);    /* copy the transactions into it */
    }
...
tbg_count (TABAG *bag)
{
    ...
    if (!z)
    {
        return -1;    /* allocate the counter arrays */
    }
    ...
    else                          /* if the items do not carry weights */
    {
        for (n = 0; n < bag->cnt; n++)
        {
            ...
            for (s = t->items; *s > TA_END; s++)
            {
                if ((i = *s) < 0)
                {
                    i = 0;    /* traverse the transaction items */
                }
...
tbg_icnts (TABAG *bag, int recnt)
{
    ...
    if ((recnt || !bag->icnts) && (tbg_count(bag) < 0))
    {
        return NULL;
    }
...
tbg_ifrqs (TABAG *bag, int recnt)
{
    ...
    if ((recnt || !bag->ifrqs) && (tbg_count(bag) < 0))
    {
        return NULL;
    }
...
tbg_add (TABAG *bag, TRACT *t)
{
    ...
    if (bag->cnt >= n)            /* if the transaction array is full */
    {
        ...
        if (!p)
        {
            return E_NOMEM;    /* enlarge the transaction array */
        }
...
int
tbg_read (TABAG *bag, TABREAD *tread, int mode)
{
    ...
    while (1)                     /* transaction read loop */
    {
        ...
        if (r < 0)
        {
            return r;    /* read the next transaction and */
        }

        if (r > 0)
        {
            return 0;    /* check for error and end of file */
        }
...
int
tbg_write (TABAG *bag, TABWRITE *twr, const char *wgtfmt, ...)
{
...
tbg_istab (TABAG *bag)
{
    ...
    if (bag->cnt <= 1)
    {
        return 0;    /* check for at most one transaction */
    }
    ...
    if (bag->mode & IB_WEIGHTS)   /* if the items carry weights */
    {
        for (n = 0; n < bag->cnt; n++)
        {
            ...
            for (a = b = x->items; a->item >= 0; a++)
            {
                ...
                if (i >= 0)
                {
                    (b++)->item = i;    /* remove all items that are */
                }
...
ITEM
tbg_recode (TABAG *bag, SUPP min, SUPP max, ITEM cnt, int dir)
{
    /* --- recode items in transactions */
    ...
    if (!map)
    {
        return -1;    /* create an item identifier map */
    }
...
void
tbg_filter (TABAG *bag, ITEM min, const int *marks, double wgt)
{
...
void
tbg_trim (TABAG *bag, ITEM min, const int *marks, double wgt)
{
        ...
        for (n = 0; n < bag->cnt; n++)
        {
            ...
            if (marks)                /* if item markers are given */
            {
                ...
                for (s = d = t->items; *s >= 0; s++)
                    if (marks[*s])
                    {
                        break;    /* trim infrequent items at the front */
                    }
...
tbg_itsort (TABAG *bag, int dir, int heap)
{
    ...
    else                          /* if the items do not carry weights */
    {
        ...
        for (n = 0; n < bag->cnt; n++)
        {
            ...
            if (k < 2)
            {
                continue;    /* do not sort less than two items */
            }
...
static void
pksort (TRACT **tracts, TRACT **buf, TID n, ITEM o)
{
    ...
    if (n <= 1)
    {
        return;    /* sort only 2 and more transactions */
    }

    if (n <= 32)                  /* sort few transactions plainly */
    {
        ptr_mrgsort(tracts, (size_t)n, +1, ta_cmpoff, &o, buf);
        return;
    }
    ...
    src = tracts;
    dst = buf;      /* get trans. source and destination */
    ...
    if (cnts[mask & 0x3f] < n)    /* if more than one bit pattern */
    {
        ...
        t = src;
        src = dst;
        dst = t;/* exchange source and destination, */
    ...
    for (k = 6; k < 31; k += 5)   /* traverse the remaining sections */
    {
        ...
        if (!x)
        {
            continue;    /* in a section, skip the section */
        }
        ...
        if (cnts[x] >= n)
        {
            continue;    /* check for only one pattern */
        }
...
static void
sort (TRACT **tracts, TID n, ITEM o,
      TRACT **buf, TID *cnts, ITEM k, ITEM mask)
{
    ...
    if (cnts[x] >= n)             /* check for only one or no item */
    {
        if (x < 0)
        {
            return;    /* if all transactions end, abort */
        }

        x = (*tracts)->items[o];    /* check for packed items */

        if ((x < 0) && (mask <= ITEM_MIN))
        {
            pksort(tracts, buf, n, o);
        }
    ...
    if ((n -= m) <= 0)
    {
        return;    /* and if no others are left, abort */
    }
    ...
    if ((*tracts)->items[o] < 0)  /* if there are packed items, sort */
    {
        ...
        if (mask <= ITEM_MIN)       /* if to treat packed items equally */
        {
            ...
            if ((n -= m) <= 0)
            {
                return;    /* and if no other transactions */
            }
        }                           /* are left, abort the function */
    }                             /* traverse the formed sections */

    if ((x = (*tracts)->items[o]) < 0)
    {
        x &= mask;
    }
    ...
    for (t = tracts; --n > 0; )   /* traverse the transactions */
    {
        ...
        if (y == x)
        {
            continue;    /* if same start item, continue */
        }
...
void
tbg_sort (TABAG *bag, int dir, int mode)
{
    ...
    if (bag->cnt < 2)
    {
        return;    /* check for at least two trans. */
    }
    ...
    if (bag->mode & IB_WEIGHTS)   /* if the items carry weights, */
    {
        ...
        {
            ptr_qsort   (bag->tracts, (size_t)n, dir, wta_cmp, NULL);
        }
    }
    ...
    else                          /* if to use failsafe functions */
    {
        ...
        {
            ptr_qsort   (bag->tracts, (size_t)n, dir, cmp, NULL);
        }
...
void
tbg_sortsz (TABAG *bag, int dir, int mode)
{
    /* --- sort a transaction bag */
    TID n;                        /* number of transactions */
    ...
    if (bag->mode & IB_WEIGHTS)   /* if the items carry weights, */
    {
        ...
        {
            ptr_qsort   (bag->tracts, (size_t)n, dir, wta_cmpsz, NULL);
        }
    }
    ...
    else                          /* if the items do not carry weights */
    {
        ...
        {
            ptr_qsort   (bag->tracts, (size_t)n, dir,  ta_cmpsz, NULL);
        }
    }                             /* use heapsort or quicksort */
}  /* tbg_sortsz() */
...
tbg_reduce (TABAG *bag, int keep0)
{
    ...
    if (bag->cnt <= 1)
    {
        return 1;    /* deal only with two or more trans. */
    }
    ...
    for (i = bag->cnt; --i > 0; )
    {
        ...
        else                        /* if transactions are not equal */
        {
            ...
            {
                free(*d);    /* check weight of old transaction */
            }
    ...
    {
        free(*d);    /* check weight of last transaction */
    }
...
tbg_pack (TABAG *bag, int n)
{
    ...
    if (n <= 0)
    {
        return;    /* if no items to pack, abort */
    }
...
tbg_occur (TABAG *bag, const ITEM *items, ITEM n)
{
    ...
    for (r = m = 0; r < k; )      /* find right boundary */
    {
        ...
        if (ta_cmpx((TRACT*)bag->tracts[m], items, n) > 0)
        {
            k = m;
        }
...
tbg_ipwgt (TABAG *bag, int mode)
{
    ...
    if (occs)                     /* if allocation succeeded */
    {
        ...
        if (bag->mode & IB_WEIGHTS) /* if the items carry weights */
        {
            ...
            for (n = bag->cnt; --n >= 0; )
            {
                ...
                if (wgts)
                {
                    memset(wgts, 0, (size_t)a->size *sizeof(double));
                }
                ...
                if (!wgts)
                {
                    continue;    /* skip item weights if not needed */
                }
    ...
    if (wgts)
    {
        free(wgts);    /* deallocate working memory */
    }
...
tbg_show (TABAG *bag)
{
    ...
    for (i = 0; i < bag->cnt; i++)
    {
        ...
        {
            ta_show( (TRACT*)bag->tracts[i], bag->base);
        }
       ...
       ITEM *flags, ITEM *offs, RNG *rng)
{
    ...
    if (k <= 0)
    {
        return;    /* check for a swapable item */
    }
...
tbg_random (TABAG *src, RNG *rng, TABAG *dst)
{
    ...
    if (!dst->buf)                /* and get the number of items */
    {
        ...
        if (!ifrq)
        {
            return NULL;    /* create an item frequency buffer */
        }
    ...
    for (k = dst->cnt; --k >= 0; )
    {
        ...
        for (i = 0; i < t->size; i++)
        {
            ...
            if (j >= m)
            {
                j = m-1;    /* ensure a proper array index */
            }
...
tbg_swap (TABAG *src, RNG *rng, TABAG *dst)
{
    ...
    if (src->cnt < 2)
    {
        return dst;    /* check for at most one transaction */
    }
    ...
    if (!dst->buf)                /* clone the memory structure */
    {
        ...
        if (!dst->buf)
        {
            return NULL;    /* create an item flag array */
        }
    ...
    for (z = (first) ? dst->extent << 1 : dst->extent >> 1; z > 0; z--)
    {
        ...
        if (i <  0)
        {
            i = 0;    /* generate first transaction index */
        }
        ...
        if (j <  0)
        {
            j = 0;    /* generate second transaction index */
        }

        if (i == j)
        {
            continue;    /* transactions must be different */
        }
...
tbg_shuffle (TABAG *src, RNG *rng, TABAG *dst)
{
    ...
    for (k = dst->cnt; --k > 0;)  /* traverse the transactions */
    {
        for (i = 0; i < n; i++)     /* traverse the items/columns */
        {
            ...
            if      (j > k)
            {
                j = k;    /* compute a random transaction index */
            }

            else if (j < 0)
            {
                j = 0;    /* and clamp it to the proper range */
            }
...
taa_collate (TRACT **taa, TID n, ITEM k)
{
    ...
    for (d = *taa, i = 0; ++i < n; )
    {
        ...
        a = d->items;
        b = s->items; /* compare packed items */
        x = (ispacked(*a)) ? *a++ : 0;
        y = (ispacked(*b)) ? *b++ : 0;
        ...
        for ( ; (UITEM)*a < (UITEM)k; a++, b++)
            if (*a != *b)
            {
                break;    /* compare transaction prefixes */
            }

        if (*a != k)
        {
            d = s;    /* if not equal, keep transaction */
        }
...
taa_uncoll (TRACT **taa, TID n)
{
    ...
    for (d = *taa, i = 0; ++i < n; )
    {
        ...
        if (s->wgt >= 0)
        {
            d = s;    /* get uncollated transactions */
        }
            ...
            const ITEM *map, void *buf, void **dst)
{
    ...
    t = *(TRACT**)dst;
    p = taa;   /* get the transaction memory */
    ...
    for (i = 0; i < n; i++)       /* traverse the transactions */
    {
        ...
        if (ispacked(*s))           /* if there are packed items, */
        {
            ...
            if (x)
            {
                *d++ = x | TA_END;    /* if there are packed items left, */
            }
        ...
        if (t->size <= 0)
        {
            continue;    /* delete empty transactions */
        }
        ...
        *p++ = t;
        t = (TRACT*)d;    /* store the filtered transaction */
    ...
    if (n >= 32)                  /* if there are enough trans. left */
    {
        if (end >= n)
        {
            ptr_mrgsort(taa, (size_t)n, +1, ta_cmp, NULL, buf);
        }

        else
        {
            sort(taa, n, 0, buf, (TID*)((TRACT*)buf+n)+1, end, -1);
        }
            ...
            const ITEM *map, void *buf, void **dst)
{
    ...
    for (i = 0; i < n; i++)       /* traverse the transactions */
    {
        ...
        if (ispacked(*s))           /* if there are packed items, */
        {
            ...
            if (m)
            {
                *d++ = m | TA_END;    /* if there are packed items left, */
            }
        }                           /* store them in the destination */
        ...
        if (t->size <= 0)
        {
            continue;    /* ignore empty transactions */
        }
        ...
        *d++ = (ITEM)h;             /* compute and store the hash value */
        ...
        for ( ; htab[k]; k = (k+x) % z)
        {
            ...
            if (m < 0)
            {
                break;    /* if collision with same trans., */
            }
        }                           /* abort probing (can combine) */

        if (htab[k])
        {
            htab[k]->wgt += taa[i]->wgt;
        }
...
static int
create (TANODE *node, TRACT **tracts, TID cnt, ITEM index)
{
    ...
    node->max = 0;
    node->wgt = 0; /* init. weight and suffix length */
    ...
    for ( ; cnt >= 0; cnt = i)    /* while there are transactions left */
    {
        ...
        for (i = cnt; i >= 0; i--)  /* find range of the next item */
            if (tracts[i]->items[index] != child->item)
            {
                break;
            }
        ...
        if (create(child, tracts+i+1, cnt-i, index+1) != 0)
        {
            ...
            node->data = NULL;
            node->max = 0;
            return -1; /* recursively fill the child nodes */
...
tat_create (TABAG *bag)
{
    ...
    if (!tree)
    {
        return NULL;    /* create the transaction tree body */
    }
...
int
tat_filter (TATREE *tree, ITEM min, const int *marks, int heap)
{
...
static void
show (TANODE *node, ITEMBASE *base, int ind)
{
...
TANODE*
create (TRACT **tracts, TID cnt, ITEM index)
{
    /* --- recursive part of tat_create() */
    TID    i;                     /* loop variable */
    ITEM   item, k, n;            /* item identifier and counter */
    ...
    if (n <= 0)
    {
        return node;    /* return the created tree */
    }
    ...
    for (--cnt; --n >= 0; cnt = i)   /* traverse the different items */
    {
        ...
        if (!chn[n])
        {
            break;    /* recursively create a subtree */
        }

        if ((k = chn[n]->max +1) > node->max)
        {
            node->max = k;
        }
    ...
    if (n < 0)
    {
        return node;    /* if successful, return created tree */
    }
...
tat_create (TABAG *bag)
{
    ...
    if (bag->cnt <= 0)            /* if the transaction bag is empty, */
    {
        ...
        tree->root->wgt = 0;
        tree->root->size = tree->root->max = 0;
    }
...
int main (int argc, char *argv[])
{
    ...
    if (optarg)
    {
        error(E_OPTARG);    /* check (option) arguments */
    }

    if (k < 1)
    {
        error(E_ARGCNT);    /* and number of arguments */
    }
    ...
    if (!ibase)
    {
        error(E_NOMEM);    /* to manage the items */
    }
    ...
    if (!tread)
    {
        error(E_NOMEM);    /* and configure the characters */
    }
    ...
    if (!tabag)
    {
        error(E_NOMEM);    /* to store the transactions */
    }
    ...
    if (m <  0)
    {
        error(E_NOMEM);    /* recode items and transactions */
    }

    if (m <= 0)
    {
        error(E_NOITEMS);    /* and check the number of items */
    }
    ...
    if (!tracts)
    {
        error(E_NOMEM);    /* copy transactions to a buffer */
    }
    ...
    if (pack)
    {
        tbg_pack(tabag,16);    /* pack 16 items with lowest codes */
    }
