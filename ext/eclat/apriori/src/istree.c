...
static ITEM
search (ITEM id, ISTNODE **chn, ITEM n)
{
    ...
    for (l = 0, r = n; l < r; )   /* while the range is not empty */
    {
        ...
        if      (id > x)
        {
            l = m+1;    /* to the middle element and */
        }

        else if (id < x)
        {
            r = m;    /* adapt the range boundaries */
        }

        else
        {
            return m;    /* if there is an exact match, */
        }
...
static ITEM
bisect (ITEM id, ISTNODE **chn, ITEM n)
{
    /* --- find a child node (index) */
    ITEM l, r, m;                 /* left, right, and middle index */
    ITEM x;                       /* item of middle child */
...
static SUPP
getsupp (ISTNODE *node, ITEM *items, ITEM n)
{
    ...
    for ( ; --n > 0; items++)     /* follow the set/path from the node */
    {
        ...
        if (k <= 0)
        {
            return SKIP;    /* the support is less than minsupp */
        }
        ...
        if (node->offset >= 0)      /* if a pure array is used */
        {
            ...
            if (i >= k)
            {
                return SKIP;    /* and check whether entry exists */
            }
        }
        ...
        if (i < 0)
        {
            return SKIP;    /* if child does not exist, abort */
        }
    ...
    if (node->offset >= 0)        /* if a pure array is used, */
    {
        ...
        if (i >= k)
        {
            return SKIP;    /* check whether counter exists */
        }
    }
    ...
    if (i < 0)
    {
        return SKIP;    /* if no counter exists, abort */
    }
...
static void
reclvls (ISTREE *ist, ISTNODE *node, ITEM lvl)
{
    ...
    if (n <= 0)
    {
        return;    /* if there are no children, abort */
    }
    ...
    if (node->offset >= 0)        /* if a pure array is used */
    {
        if (node->chcnt == 0)       /* if this is a new node (leaf) */
        {
            ...
            while ((n > 0) && (*items < o))
            {
                n--;
                items++;
            }         /* skip items before first counter */
        ...
        else if (node->chcnt > 0)   /* if there are child nodes */
        {
            ...
            while ((n >= min) && (*items < o))
            {
                n--;
                items++;
            }         /* skip items before the first child */
    ...
    else                          /* if an identifer map is used */
    {
        if (node->chcnt == 0)       /* if this is a new node (leaf) */
        {
            ...
            for (i = 0; --n >= 0; items++)    /* traverse the items */
            {
                if (*items > o)
                {
                    return;    /* if beyond last item, abort */
                }
        ...
        else if (node->chcnt > 0)   /* if there are child nodes */
        {
            ...
            for (--min; --n >= min; )
            {
                if (*items > o)
                {
                    return;    /* traverse the transaction */
                }
...
static void
countx (ISTNODE *node, const TANODE *tan, ITEM min)
{
    ...
    if (n <= 0)                   /* if this is a leaf node */
    {
        if (n < 0)
        {
            count(node, tan_suffix(tan), k, tan_wgt(tan), min);
        }
    ...
    if (node->offset >= 0)        /* if a pure array is used */
    {
        if (node->chcnt == 0)       /* if this is a new node (leaf) */
        {
            ...
            for (cld = tan_children(tan); cld; cld = tan_sibling(cld))
            {
                ...
                if (i < 0)
                {
                    return;    /* if before first item, abort */
                }

                if (i < node->size)
                {
                    INC(node->cnts[i], tan_wgt(cld));
                }
            }
    ...
    else                          /* if an identifer map is used */
    {
        if (node->chcnt == 0)       /* if this is a new node (leaf) */
        {
            ...
            for (cld = tan_children(tan); cld; cld = tan_sibling(cld))
            {
                ...
                if (item < o)
                {
                    return;    /* if before the first item, return */
                }
                ...
                {
                    k++;    /* if the corresp. counter exists, */
                }
        ...
        else if (node->chcnt > 0)   /* if there are child nodes */
        {
            ...
            for (cld = tan_children(tan); cld; cld = tan_sibling(cld))
            {
                ...
                if (item < o)
                {
                    return;    /* if before the first item, abort */
                }
...
countx (ISTNODE *node, const TANODE *tan, ITEM min)
{
    ...
    if (n <= 0)                   /* if there are no children */
    {
        if (n < 0)
        {
            count(node, tan_items(tan), -n, tan_wgt(tan), min);
        }
    ...
    else                          /* if an identifer map is used */
    {
        if (node->chcnt == 0)       /* if this is a new node (leaf) */
        {
            ...
            for (n = tan_size(tan); --n >= 0; )
            {
                ...
                if (i >= 0)
                {
                    k = i;    /* if counter exists, add trans. wgt. */
                ...
                {
                    k++;    /* add the transaction weight to it, */
                }
        ...
        else if (node->chcnt > 0)   /* if there are child nodes */
        {
            ...
            for (--min, n = tan_size(tan); --n >= 0; )
            {
                ...
                if (i >= 0)
                {
                    countx(chn[k = i], tan_child(tan, n), min);
                }
...
static double
evaluate (ISTREE *ist, ISTNODE *node, ITEM index)
{
    ...
    *--path = item;
    ...
    for ( ; curr; curr = curr->parent)
    {
        ...
        {
            agg += val;    /* compute the rule evaluation */
        }

        *--path = item;
        n += 1;     /* and aggregate it by min/max/sum */
...
ISTREE*
ist_create (ITEMBASE *base, int mode,
            SUPP smin, SUPP body, double conf)
{
    ...
    if (!ist)
    {
        return NULL;    /* allocate the tree body */
    }
    ...
    ist->ndcnt  = 1;
    ist->ndprn = ist->mapsz = 0;
    ist->sccnt  = ist->scnec = n;
    ist->scprn = 0;
...
void
ist_count (ISTREE *ist, const ITEM *items, ITEM n, SUPP wgt)
{
    /* --- count a transaction */
...
void
ist_countt (ISTREE *ist, const TRACT *t)
{
    /* --- count a transaction */
...
void
ist_countb (ISTREE *ist, const TABAG *bag)
{
...
void
ist_countx (ISTREE *ist, const TATREE *tree)
{
...
static int
used (ISTNODE *node, int *marks, SUPP supp)
{
...
ITEM
ist_check (ISTREE *ist, int *marks)
{
    /* --- check item usage */
...
ist_prune (ISTREE *ist)
{
    ...
    for (node = ist->lvls[ist->height-1]; node; node = node->succ)
    {
        ...
        else                        /* if an identifier map is used */
        {
            ...
            if (k <= 0)
            {
                continue;    /* if no items were pruned, continue */
            }
    ...
    for (np = ist->lvls +ist->height-1; *np; )
    {
        ...
        *np = node->succ;
        free(node); /* remove empty nodes */
#ifdef BENCH                /* if benchmark version */
        ist->ndcnt--;
        ist->ndprn++; /* update the number nodes */
...
static ISTNODE*
child (ISTREE *ist, ISTNODE *node, ITEM index, SUPP pex)
{
    ...
    if (!curr)
    {
        return (ISTNODE*)-1;    /* create a child node */
    }

    if (hdonly)
    {
        item |= HDONLY;    /* set the head only flag and */
    }
...
static ISTNODE**
children (ISTREE *ist, ISTNODE **np, ISTNODE **end)
{
    ...
    frst = end;
    last = NULL;      /* note start of the child node list */

    if (!(ist->mode & IST_PERFECT))
    {
        pex = SUPP_MAX;
    }
    ...
    {
        pex = getsupp(node->parent, &node->item, 1);
    }
    ...
    for (i = n = 0; i < node->size; i++)
    {
        ...
        if (!cur)
        {
            continue;    /* create a child node if necessary */
        }
    ...
    *end = NULL;                  /* terminate the child node list */
    ...
    chn = np;
    par = node->parent; /* get the parent node */
    ...
    if (!node)
    {
        return NULL;    /* add a child array to the node */
    }
...
needed (ISTNODE *node)
{
    ...
    if (r)
    {
        return -1;    /* recursively check all children */
    }
...
ist_up (ISTREE *ist)
{
    ...
    if (!ist->curr->parent)
    {
        return;    /* check for a parent node */
    }
...
ist_down (ISTREE *ist, ITEM item)
{
    ...
    else                          /* if an identifier map is used */
    {
        ...
        if (i < 0)
        {
            return -1;    /* search for the item in the map */
        }
...
ist_next (ISTREE *ist, ITEM item)
{
    ...
    if (node->offset >= 0)        /* if a pure array is used, */
    {
        ...
        return item +1;
    }           /* return the next item identifier */
    ...
    else                          /* if an identifier map is used */
    {
        ...
        if (i >= 0)
        {
            i++;    /* try to find the item in the map */
        }
...
SUPP
ist_getsupp (ISTREE *ist, ITEM item)
{
    ...
    if (i < 0)
    {
        return 0;    /* abort if item not found */
    }
...
SUPP
ist_setsupp (ISTREE *ist, ITEM item, SUPP supp)
{
    /* --- get support for an item */
    ITEM    i;                    /* array index */
    ITEM    *map;                 /* item identifier map */
...
SUPP
ist_incsupp (ISTREE *ist, ITEM item, SUPP supp)
{
    /* --- get support for an item */
    ITEM    i;                    /* array index */
    ITEM    *map;                 /* item identifier map */
...
ist_addchn (ISTREE *ist)
{
    ...
    if (!end)
    {
        return -1;    /* add children to the current node */
    }
...
SUPP
ist_supp (ISTREE *ist, ITEM *items, ITEM n)
{
    /* --- get support of an item set */
    assert(ist                    /* check the function arguments */
           &&    (n >= 0) && (items || (n <= 0)));
...
void
ist_filter (ISTREE *ist, ITEM size)
{
    ...
    if      (size < 0)            /* -- weak filtering with evaluation */
    {
        if (size > -2)
        {
            size = -2;    /* traverse the tree levels */
        }
            ...
            for (node = ist->lvls[h]; node; node = node->succ)
            {
                ...
                for (i = node->size; --i >= 0; )
                {
                    ...
                    *--path = ITEMOF(node);   /* initialize the path */
                    ...
                    {
                        if (getsupp(curr, path+1, n) >= ist->smin)
                        {
                            break;
                        }
...
static void
clear (ISTNODE *node, ITEM *items, ITEM n, SUPP supp)
{
...
void
ist_clomax (ISTREE *ist, int target)
{
                    ...
                    for (curr = node; curr; curr = curr->parent)
                    {
                        if (curr->offset >= 0)     /* if a pure array is used */
                        {
                            ...
                            while (++k < curr->size)    /* traverse succ. items */
                            {
                                ...
                                if (getsupp(curr, path-1, n+1) >= supp)
                                {
                                    break;
                                }
                            }
...
void
ist_setsize (ISTREE *ist, ITEM zmin, ITEM zmax)
{
...
void
ist_seteval (ISTREE *ist, int eval, int agg,
             double thresh, ITEM prune)
{
    /* --- set additional evaluation */
    assert(ist);                  /* check the function arguments */
    ist->invbxs = eval & IST_INVBXS;
    eval &= ~IST_INVBXS;
...
static int
emptyset (ISTREE *ist, SUPP *supp, double *eval)
{
...
ITEM
ist_iset (ISTREE *ist, ITEM *set, SUPP *supp, double *eval)
{
    ...
    if (supp)
    {
        *supp = curr;    /* store the item set support and */
    }

    if (eval)
    {
        *eval = val;    /* the value of the add. measure */
    }
          ...
          SUPP *sset, SUPP *sbody, SUPP *shead, double *eval)
{
    ...
    while (1)                     /* search for a rule */
    {
        ...
        if (!refn)                  /* if no add. eval. measure given, */
        {
            val = 0;
            break;
        }         /* abort the loop (select the rule) */
    ...
    if (sset)
    {
        *sset  = supp;    /* store the rule support values */
    }

    if (sbody)
    {
        *sbody = body;    /* (whole rule and only body) */
    }

    if (shead)
    {
        *shead = head;    /* store the head item support, */
    }
...
static int
isets (ISTREE *ist, ISREPORT *rep, ISTNODE *node, SUPP supp)
{
...
static int
r4set (ISTREE *ist, ISREPORT *rep, ISTNODE *node, ITEM index)
{
    ...
    do                            /* check and report the first rule */
    {
        ...
        if (!refn)
        {
            val = 0;    /* if no add. eval. measure given, */
        }
        ...
        if (isr_reprule(rep, item, body, head, val) != 0)
        {
            return -1;
        }
    }
    ...
    ist->path = ist->buf +ist->height;
    *--ist->path = item;
    n = 1;   /* store head item on the path */
...
static int
rules (ISTREE *ist, ISREPORT *rep, ISTNODE *node)
{
...
int
ist_report (ISTREE *ist, ISREPORT *rep, int target)
{
    ...
    if (target & ISR_RULES)       /* if to report association rules */
    {
        ...
        else                        /* if a size order is requested */
        {
            while (1)                 /* extract assoc. rules from tree */
            {
                ...
                if (k < 0)
                {
                    break;    /* get the next association rule */
                }
                ...
                if (r < 0)
                {
                    break;    /* report the extracted ass. rule */
                }
    ...
    else                          /* if to report frequent item sets */
    {
        ...
        else                        /* if a size order is requested */
        {
            while (1)                 /* extract item sets from the tree */
            {
                ...
                if (k < 0)
                {
                    break;    /* get the next frequent item set */
                }
                ...
                if (r < 0)
                {
                    break;    /* report the extracted item set */
                }
...
double
ist_evalx (ISREPORT *rep, void *data)
{
    /* --- evaluate current item set */
...
static void
showtree (ISTNODE *node, ITEMBASE *base, ITEM level)
{
    /* --- show subtree */
    ...
    for (i = 0; i < node->size; i++)
    {
        ...
        if (cnt <= 0)
        {
            continue;    /* check whether there are children */
        }
