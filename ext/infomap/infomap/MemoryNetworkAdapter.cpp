...
bool
MemoryNetworkAdapter::readExternalHierarchy(std::string filename)
{
...
void
MemoryNetworkAdapter::readHumanReadableTree(std::string filename)
{
    std::string line;
    std::string buf;
    SafeInFile input(filename.c_str());
...
void
MemoryNetworkAdapter::generateMemoryNodeMap()
{
