...
void
InfomapBase::run(Network& input, HierarchicalNetwork& output)
{
...
double
InfomapBase::partitionAndQueueNextLevel(PartitionQueue& partitionQueue, bool tryIndexing)
{
...
void
InfomapBase::queueTopModules(PartitionQueue& partitionQueue)
{
...
void
InfomapBase::queueLeafModules(PartitionQueue& partitionQueue)
{
...
void
InfomapBase::tryIndexingIteratively()
{
 ...
 */
unsigned int
InfomapBase::findSuperModulesIterativelyFast(PartitionQueue& partitionQueue)
{
...
bool
InfomapBase::processPartitionQueue(PartitionQueue& queue, PartitionQueue& nextLevelQueue, bool tryIndexing)
{
...
void
InfomapBase::sortPartitionQueue(PartitionQueue& queue)
{
...
void
InfomapBase::partition(unsigned int recursiveCount, bool fast, bool forceConsolidation)
{
...
void
InfomapBase::fineTune(bool leafLevel)
{
 ...
 */
void
InfomapBase::coarseTune(unsigned int recursiveCount)
{
    if (numTopModules() == 1)
    {
        return;
...
void
InfomapBase::partitionEachModule(unsigned int recursiveCount, bool fast)
{
...
void
InfomapBase::partitionEachModuleParallel(unsigned int recursiveCount, bool fast)
{
#ifndef _OPENMP
...
void
InfomapBase::initMemoryNetwork(MemNetwork& network)
{
    if (!network.isFinalized())
    {
...
void
InfomapBase::initSuperNetwork(NodeBase& parent)
{
...
void
InfomapBase::setActiveNetworkFromChildrenOfRoot()
{
...
void
InfomapBase::setActiveNetworkFromLeafModules()
{
...
void
InfomapBase::setActiveNetworkFromLeafs()
{
...
void
InfomapBase::initPreClustering(bool printResults)
{
...
bool
InfomapBase::checkAndConvertBinaryTree()
{
...
void
InfomapBase::printNetworkData(std::string filename)
{
    printNetworkData(m_ioNetwork, filename);
}

void
InfomapBase::printNetworkData(HierarchicalNetwork& output, std::string filename)
{
...
void
InfomapBase::printHierarchicalData(HierarchicalNetwork& hierarchicalNetwork, std::string filename)
{
    if (filename.empty())
    {
        filename = m_config.outName;
...
void
InfomapBase::sortTree()
{
    sortTree(*root());
}

unsigned int
InfomapBase::printPerLevelCodelength(std::ostream& out)
{
...
void
InfomapBase::aggregatePerLevelCodelength(std::vector<PerLevelStat>& perLevelStat, unsigned int level)
{
    aggregatePerLevelCodelength(*root(), perLevelStat, level);
}

void
InfomapBase::aggregatePerLevelCodelength(NodeBase& parent, std::vector<PerLevelStat>& perLevelStat, unsigned int level)
{
...
DepthStat
InfomapBase::calcMaxAndAverageDepth()
{
    DepthStat stat;
...
void
InfomapBase::calcMaxAndAverageDepthHelper(NodeBase& root, unsigned int& maxDepth, double& sumLeafDepth,
