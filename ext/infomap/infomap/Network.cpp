...
void
Network::readInputData(std::string filename)
{
    if (filename.empty())
    {
...
unsigned int
Network::addNodes(const std::vector<std::string>& names)
{
...
void
Network::parsePajekNetwork(std::string filename)
{
...
void
Network::parseLinkList(std::string filename)
{
    if (m_config.parseWithoutIOStreams)
    {
...
std::string
Network::skipUntilHeader(std::ifstream& file)
{
    std::string line;
...
std::string
Network::parseVertices(std::ifstream& file, bool required)
{
    std::string line;
...
std::string
Network::parseVertices(std::ifstream& file, std::string header, bool required)
{
    std::istringstream ss;
    std::string buf;
    ss.str(header);
...
std::string
Network::parseLinks(std::ifstream& file)
{
    std::string line;
...
std::string
Network::parseBipartiteLinks(std::ifstream& file)
{
    std::string line;
...
void
Network::parseLink(const std::string& line, unsigned int& n1, unsigned int& n2, double& weight)
{
    m_extractor.clear();
...
void
Network::parseLink(char line[], unsigned int& n1, unsigned int& n2, double& weight)
{
    char *cptr;
...
bool
Network::parseBipartiteLink(const std::string& line, unsigned int& featureNode, unsigned int& node, double& weight)
{
...
bool
Network::addLink(unsigned int n1, unsigned int n2, double weight)
{
    ++m_numLinksFound;
...
bool
Network::addBipartiteLink(unsigned int featureNode, unsigned int node, bool swapOrder, double weight)
{
    ++m_numLinksFound;
...
bool
Network::insertLink(unsigned int n1, unsigned int n2, double weight)
{
    ++m_numLinks;
...
bool
Network::insertNode(unsigned int nodeIndex)
{
    return m_nodes.insert(nodeIndex).second;
}

void
Network::finalizeAndCheckNetwork(bool printSummary, unsigned int desiredNumberOfNodes)
{
    m_isFinalized = true;
...
void
Network::generateOppositeLinkMap(LinkMap& oppositeLinks)
{
...
void
Network::printParsingResult(bool onlySummary)
{
...
std::string
Network::getParsingResultSummary()
{
    std::ostringstream o;
...
void
Network::printNetworkAsPajek(std::string filename) const
{
...
void
Network::printStateNetwork(std::string filename) const
{
    SafeOutFile out(filename.c_str());
